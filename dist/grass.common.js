...
function random (max = 100000, min = 0, fractionDigits = 0) {
...
function runExecuteContext (runCode, directName, tagName, comp, callback) {
  try {
    ...
    const state = scope$1.insertChain(comp.state);
...
function parseStaticNode (node, comp, vnodeConf$$1) {
  ...
  `;
  vnodeConf$$1.content = runExecuteContext(code, 'Text expression("{{}}")', vnodeConf$$1.tagName, comp);
}
...
function createCompVnode (compConf, comp) {
...
function createVnode (parentConf, comp) {
  const vnodeConf$$1 = complierAst(comp.constructor.$ast, comp);
...
function createAst (comp) {
  ...
  if (!isString(template)) {
    warn(error(`Component template must a "string" or "function", But now is "${typeof template}"
      \n\n  --->  ${comp.name}\n`));
    return
  }
