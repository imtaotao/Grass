...
function getWeight(directive) {
  var wight = directWeight[directive];
  if (directive.includes('v-bind')) wight = BIND;
  if (directive.includes('v-on')) wight = ON;
  if (directive.includes('v-transition')) wight = TRANSITION$1;
  return wight;
}
function isReservedDirective(directive) {
  return directive.includes('v-') && getWeight(directive) !== undefined;
}
function isTransitionHook(directive) {
  return TRANSITIONHOOK.includes(directive);
}
function splitDirective(directiveKey) {
  var args = directiveKey.split('.');
  var directive = args[0];
  var modifiers = args.splice(1);
  return { directive: directive, modifiers: modifiers };
}
  ...
  if (isReservedTag(vnodeConf.tagName)) {
    ...
    try {
      for (var _iterator = events[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        ...
        var _splitDirective = splitDirective(direactiveKey),
            name = _splitDirective.directive,
            modifiers = _splitDirective.modifiers;
